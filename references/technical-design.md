...
PPT Master Plus is not PPT-only — the same SVG → DrawingML pipeline produces square posters, 9:16 stories, A4 prints. Format-specific conventions (ratios, safe zones, brand areas) live in [`canvas-formats.md`](canvas-formats.md).
...
The catalog of *how an image is placed on a slide* (full vocabulary in [`image-layout-patterns.md`](image-layout-patterns.md)) splits 72 numbered techniques into two layers that compose freely:
...
This is the key insight that's easy to miss when reading the code. The same modules under `skills/ppt-master-plus/scripts/svg_finalize/` are used in two places, for two different products.
