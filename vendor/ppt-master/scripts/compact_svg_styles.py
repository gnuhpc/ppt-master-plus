...
def _normalize_root_font_family(
    ...
    declarations = _style_declarations(root.get("style")) or []
...
def _ensure_root_font_family(root: ET.Element, stats: StyleCompactionStats) -> None:
    """Guarantee a stable root default for projected visible text."""
    if (root.get("font-family") or "").strip():
        return
    if any(
        _local_name(element.tag) == "text" and "".join(element.itertext()).strip()
        for element in root.iter()
    ):
...
def compact_svg_style_tree(root: ET.Element) -> StyleCompactionStats:
    ...
    _ensure_root_font_family(root, stats)
