...
分区和关系由 [`executor-structure.md`](../references/ppt-master-core/executor-structure.md) 在当前页面
...
| 效果与兼容输入 | [`svg-effects.md`](../references/ppt-master-core/svg-effects.md) |
| Native Chart/Table | [`native-data-interface.md`](../references/ppt-master-core/native-data-interface.md) |
...
[`executor-chart.md`](../references/ppt-master-core/executor-chart.md) 定义的 semantic object key
和 scoped marker；不能在同一 SVG 中复制多个 `chartArea` id。

### 6.2 Native Chart/Table

**硬规则**：只有 [`native-data-interface.md`](../references/ppt-master-core/native-data-interface.md) 支持的真实 Chart 或纯文本 Table 使用 replacement marker。JSON metadata 与可见 fallback 必须表达同一份数据。运行时构建的 Structure 永远不使用 Chart/Table replacement marker。
