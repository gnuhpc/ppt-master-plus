...
- **Annotate** (changes that need AI judgement / re-layout): select an element → write the instruction → click **Add annotation** to stage it → click **Apply changes** to write annotation markers → the success dialog shows a one-click-copy prompt containing the affected page number(s); paste that prompt into the AI tool chat so the AI modifies those pages from the annotations saved in `svg_output/`.
...
- **Buttons**: `Add annotation` stages annotation text in memory; `Apply changes` writes staged direct edits plus annotation markers to disk and keeps the service running. If annotations were saved, the success dialog includes a copyable prompt with the affected page number(s), instructing the user to paste it into the AI tool chat. `Exit preview` is the only UI action that stops Flask.
