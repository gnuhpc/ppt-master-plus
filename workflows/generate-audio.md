...
- High-quality cloud mode: provider API key is set before use:
  ...
  - Keys may live in the current process environment or the first `.env` found in this order: current working directory, skill directory (e.g. `~/.agents/skills/ppt-master-plus/.env`), clone repo root, `~/.ppt-master-plus/.env`, then legacy fallback `~/.ppt-master/.env`
...
```bash
python3 skills/ppt-master-plus/scripts/notes_to_audio.py --list-voices --locale <locale>
```

**ElevenLabs backend**:

```bash
python3 skills/ppt-master-plus/scripts/notes_to_audio.py --provider elevenlabs --list-voices
```

**Cloud providers using explicit voice IDs/names**:

```bash
python3 skills/ppt-master-plus/scripts/notes_to_audio.py --provider minimax --list-voices
python3 skills/ppt-master-plus/scripts/notes_to_audio.py --provider qwen --list-voices
python3 skills/ppt-master-plus/scripts/notes_to_audio.py --provider cosyvoice --list-voices
```
...
python3 skills/ppt-master-plus/scripts/notes_to_audio.py <project_path> \
...
- (If skipped embedding) one-line hint on how to embed later: `python3 skills/ppt-master-plus/scripts/svg_to_pptx.py <project_path> --recorded-narration audio`.
