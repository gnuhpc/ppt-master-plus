...
Default to **deck** unless the user explicitly says "structure only" / "layout only" / "no brand identity". When in doubt, lean deck — losing identity later is easy; reconstructing it from a layout-mode strip is not. See [`docs/zh/templates-architecture.md`](../references/templates-architecture.md) for the full kind / schema / fusion model.
...
```bash
python3 skills/ppt-master-plus/scripts/pptx_template_import.py "<reference_template.pptx>"
```
...
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<import_workspace>/svg" --icons-dir "<import_workspace>/icons" --inplace --id-prefix layered --min-decoration-bytes 3000 --clean-stale

# mirror path
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<import_workspace>/svg-flat" --icons-dir "<import_workspace>/icons" --inplace --id-prefix flat --min-decoration-bytes 3000 --clean-stale
```
...
```bash
python3 skills/ppt-master-plus/scripts/extract_svg_assets.py "<svg_analysis_workspace>/svg" --icons-dir "<svg_analysis_workspace>/icons" --inplace --id-prefix source --min-decoration-bytes 3000 --clean-stale
```
...
```bash
mkdir -p "skills/ppt-master-plus/templates/layouts/<template_id>"
```

> **Output location**: Global templates go to `skills/ppt-master-plus/templates/layouts/`; project templates go to `projects/<project>/templates/`
>
...
ls -la "skills/ppt-master-plus/templates/<kind_dir>/<template_id>"
```

Run SVG validation on the template directory:

```bash
python3 skills/ppt-master-plus/scripts/svg_quality_checker.py "skills/ppt-master-plus/templates/<kind_dir>/<template_id>" --template-mode --format <canvas_format>
```
...
```bash
# For deck (default)
python3 skills/ppt-master-plus/scripts/register_template.py <template_id> --kind deck

# For layout
python3 skills/ppt-master-plus/scripts/register_template.py <template_id> --kind layout
```
...
>
> ```bash
> python3 skills/ppt-master-plus/scripts/register_template.py --kind deck --rebuild-all
> python3 skills/ppt-master-plus/scripts/register_template.py --kind layout --rebuild-all
> ```
