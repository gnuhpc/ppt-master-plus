...
2. First `.env` found in this order:
   ...
   - Skill directory (e.g. `~/.agents/skills/ppt-master-plus/.env`)
   - Clone repo root
   - `~/.ppt-master-plus/.env`
   - Legacy fallback: `~/.ppt-master/.env`
...
`image_search.py` uses the same `.env` lookup order as `image_gen.py`, so skill installs can keep `PEXELS_API_KEY` / `PIXABAY_API_KEY` in `~/.ppt-master-plus/.env`.
